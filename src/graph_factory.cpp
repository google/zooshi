...
static bool ParseGraphFlatbuffer(event::EventSystem* event_system,
                                 event::Graph* graph,
                                 const graph::GraphDef* graph_def,
                                 pindrop::AudioEngine* audio_engine) {
  ...
  for (size_t node_index = 0; node_index != graph_def->node_list()->size();
    ...
    if (edges != nullptr) {
      for (size_t edge_index = 0; edge_index < edges->size(); ++edge_index) {
        ...
        ParseType(node_index, edge_index, graph, edge_def, audio_engine);
      }
    }
  }
  return true;
...
event::Graph* LoadGraph(event::EventSystem* event_system,
                        GraphDictionary* graph_dictionary, const char* filename,
                        pindrop::AudioEngine* audio_engine) {
  // Have we loaded this flatbuffer already?
  if (graph_dictionary->count(filename) != 0) {
    // If so, return the cached graph.
    return (*graph_dictionary)[filename].get();
  } else {
    ...
    (*graph_dictionary)[filename].reset(graph);
    const graph::GraphDef* graph_def = graph::GetGraphDef(source.c_str());
    if (!ParseGraphFlatbuffer(event_system, graph, graph_def, audio_engine)) {
      return nullptr;
    }
    return graph;
  }
